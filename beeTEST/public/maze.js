...
Maze.SKIN = {
    ...
    //Never called since no obstacle
...
for (var kind in Maze.mapCells) { //For each kind of cell
    if(kind != "cloud"){
        for(var cell of Maze.mapCells[kind]){
            if(cell.optional){ //If this cell is optional, generate a random number to remove it or not
                var val = Math.round(Math.random());
                if(val == 0) //0, let the cell
                    cell.remainingValue = cell.value;
                else{ //1, remove it
                    var index = Maze.mapCells[kind].indexOf(cell);
                    if (index > -1)
                      Maze.mapCells[kind].splice(index, 1);
                }
            }
            else if(cell.or){ //If this cell is either this kind or another
                var val = Math.round(Math.random());
                if(val == 0) //0, let the cell
                    cell.remainingValue = cell.value;
                else{ //1, change the type
                    cell.remainingValue = cell.value;
                    var index = Maze.mapCells[kind].indexOf(cell);
                    if (index > -1)
                      Maze.mapCells[kind].splice(index, 1);
                    Maze.mapCells[cell.or].push(cell)
                }
            }
            else
                cell.remainingValue = cell.value;
...
Maze.SQUARE_SIZE = json.visuals.squareSize;
Maze.PEGMAN_HEIGHT = json.visuals.avatarHeight;
Maze.PEGMAN_WIDTH = json.visuals.avatarWidth;
...
Maze.reveal = function(){
    for(var cell of Maze.mapCells["purpleFlower"]){
        var min = cell.range[0];
        var max = cell.range[1];
        var val = Math.floor(Math.random() * (max - min + 1)) + min;; //Pick a random number in range
        cell.value = val;
